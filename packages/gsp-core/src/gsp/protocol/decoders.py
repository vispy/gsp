    ...
    if fortran_order is not False and not policy.allow_fortran_order:
        diagnostics.append(
            SecurityDiagnostic(
                ...
                "S022 .npy decoder requires fortran_order=false",
            )
        )
...
def _parse_npy_header(
    ...
    if not payload.startswith(b"\x93NUMPY"):
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, "invalid .npy magic prefix"
            )
        )
        return None
    if len(payload) < 10:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, ".npy payload is too short"
            )
        )
        return None
    ...
    if version not in policy.allowed_versions:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                f".npy version {version!r} is not allowed",
            )
        )
        return None
    ...
    if len(payload) < header_offset:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                ".npy payload is missing header length",
            )
        )
        return None
    ...
    if header_length > policy.max_header_bytes:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_LIMIT_EXCEEDED, ".npy header exceeds max_header_bytes"
            )
        )
        return None
    ...
    if len(payload) < data_offset:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                ".npy payload is shorter than declared header",
            )
        )
        return None
    ...
    try:
        ...
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, f".npy header parse failed: {exc}"
            )
        )
        return None
    if not isinstance(header_value, dict):
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, ".npy header must be a dictionary"
            )
        )
        return None
    ...
    if set(header) != {"descr", "fortran_order", "shape"}:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, ".npy header has unexpected keys"
            )
        )
        return None
...
def _dtype_from_header(
    value: object, diagnostics: list[SecurityDiagnostic]
) -> np.dtype[Any] | None:
    if not isinstance(value, str):
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, ".npy descr must be a string"
            )
        )
        return None
    try:
        ...
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, f".npy dtype is invalid: {exc}"
            )
        )
        return None
    return dtype
...
def _shape_from_header(
    ...
    if not isinstance(value, tuple) or not value:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                ".npy shape must be a non-empty tuple",
            )
        )
        return None
    if not all(isinstance(dim, int) and dim > 0 for dim in value):
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                ".npy shape dimensions must be positive integers",
            )
        )
        return None
    return tuple(value)
...
def _validate_dtype(
    ...
    if dtype.name not in policy.allowed_dtypes:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                f".npy dtype {dtype.name!r} is not allowed",
            )
        )
    if dtype.hasobject or dtype.kind == "O":
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                ".npy object dtype and pickle payloads are rejected",
            )
        )
    if dtype.fields is not None:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, ".npy structured dtype is rejected"
            )
        )
    if dtype.kind in ("S", "U", "V"):
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                ".npy string, unicode, and void dtypes are rejected",
            )
        )
    if dtype.byteorder == ">":
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, ".npy big-endian dtype is rejected"
            )
        )
    if dtype.name != policy.expected_dtype:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                ".npy dtype does not match expected dtype",
            )
        )
...
def _validate_shape(
    ...
    if len(shape) > policy.max_rank:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID, ".npy rank exceeds max_rank"
            )
        )
    if shape != policy.expected_shape:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                ".npy shape does not match expected shape",
            )
        )
    elements = prod(shape)
    if elements > policy.max_elements:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_LIMIT_EXCEEDED,
                ".npy element count exceeds max_elements",
            )
        )
...
def _validate_payload_size(
    ...
    if decoded_bytes > policy.max_decoded_bytes:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.DECOMPRESSION_LIMIT_EXCEEDED,
                ".npy decoded bytes exceed max_decoded_bytes",
            )
        )
    ...
    if len(payload) != expected_total:
        diagnostics.append(
            SecurityDiagnostic(
                SecurityDiagnosticCode.CHUNK_METADATA_INVALID,
                ".npy payload size does not match header shape and dtype",
            )
        )
