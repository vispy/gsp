...
def test_plot_logical_pixel_plot_ndc_round_trip(
    ...
    ndc = plot_logical_px_to_plot_ndc(snapshot, center)

    assert plot_ndc_to_plot_logical_px(snapshot, ndc) == pytest.approx(center)
    assert plot_ndc_to_plot_logical_px(snapshot, (-1.0, 1.0)) == pytest.approx((plot.x, plot.y))
    assert plot_ndc_to_plot_logical_px(snapshot, (1.0, -1.0)) == pytest.approx(
        (plot.x + plot.width, plot.y + plot.height)
    )


def test_bottom_left_pixel_origin_round_trip_preserves_plot_ndc_orientation() -> None:
    snapshot = _snapshot(origin=PixelOrigin.BOTTOM_LEFT)

    assert plot_ndc_to_plot_logical_px(snapshot, (-1.0, -1.0)) == pytest.approx((0.0, 0.0))
    assert plot_logical_px_to_plot_ndc(snapshot, (800.0, 600.0)) == pytest.approx((1.0, 1.0))
...
def test_exact_plot_corners_center_and_closed_edges(origin: PixelOrigin) -> None:
    ...
    assert plot_ndc_to_plot_logical_px(snapshot, (-1.0, top_y_ndc)) == pytest.approx((100.0, 75.0))
    assert plot_ndc_to_plot_logical_px(snapshot, (1.0, bottom_y_ndc)) == pytest.approx(
        (700.0, 525.0)
    )
    assert plot_ndc_to_plot_logical_px(snapshot, (0.0, 0.0)) == pytest.approx((400.0, 300.0))
...
def test_resolved_plot_aspect_and_three_way_coordinate_classification() -> None:
    ...
    assert (
        classify_logical_coordinate(snapshot, (140.0, 110.0)) is LogicalCoordinateRegion.DATA_PLOT
    )
    assert (
    ...
    assert (
        classify_logical_coordinate(snapshot, (90.0, 80.0)) is LogicalCoordinateRegion.OUTSIDE_PANEL
    )
...
def test_resolved_snapshot_rejects_empty_data_viewport() -> None:
    with pytest.raises(ValueError, match="positive width and height"):
        _snapshot(plot=LogicalPixelRect(100.0, 100.0, 0.0, 200.0))
...
def test_plot_conversion_rejects_guide_lane_instead_of_extrapolating() -> None:
    ...
    with pytest.raises(ValueError, match="inside the closed plot_rect_px"):
        plot_logical_px_to_plot_ndc(snapshot, (120.0, 80.0))
