...
def test_scene_rejects_both_active_views() -> None:
    with pytest.raises(ValueError, match="cannot define both view2d and view3d"):
        _scene(id="scene:invalid", view2d=_view2d(), view3d=_view3d())


def test_scene_accepts_viewless_ndc_or_one_active_view() -> None:
    assert _scene(id="scene:viewless").view2d is None
    assert _scene(id="scene:2d", view2d=_view2d()).view2d is not None
    assert _scene(id="scene:3d", view3d=_view3d()).view3d is not None
...
def test_scene_accepts_text_billboard3d_only_with_data_view3d_contract() -> None:
    ...
    assert _scene(id="scene:billboard", visuals=(billboard,), view3d=_view3d()).visuals == (
        billboard,
    )
    ...
    with pytest.raises(ValueError, match="positions3d require CoordinateSpace.DATA"):
        _scene(id="scene:ndc3", visuals=(ndc3,), view3d=_view3d())
    ...
    with pytest.raises(ValueError, match="does not support a 2D visual transform"):
        _scene(id="scene:transform3", visuals=(transformed,), view3d=_view3d())
